...
func TestBroadcastMsgSubscribeAndUnSubscribeOnMetaRoom(t *testing.T) {
	...
	ds.removeClientFromRoom("test9", roomName)
	...
	ds.addClientToRoom("test9", roomName)
	ctx, cancel = context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	ds.broadcastMsg(ctx, dummyClient, roomName, msg)
	testClientRead(t)
...
func TestBroadcastAddDeleteClientOnMetaRoom(t *testing.T) {
	t.Parallel()
	ds := newChatDataStore(ioutil.Discard)
	var err error
	const roomName = "default"
	...
	// Unsubscribe one client.
	ds.ignoreNamedClient("test9", dummyClient)
	// again broadcast to the all the servers on meta room
	ctx, cancel = context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	ds.broadcastMsg(ctx, dummyClient, roomName, msg)
	for i, client := range clients {
		err := client.SetReadDeadline(time.Now().Add(time.Millisecond * 10))
		if err != nil {
			t.Errorf("SetReadDeadline failed: %v\n", err)
		}
		b := make([]byte, 8)
	...
	//
	// subscribe the client again.
	ds.allowNamedClient("test9", dummyClient)
	ctx, cancel = context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()
	ds.broadcastMsg(ctx, dummyClient, roomName, msg)
	testClientRead(t)
